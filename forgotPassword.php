<?php
	if (isset($_POST["forgotPass"])) {
		...
		$data = $connection->query("SELECT id FROM register WHERE email='$email'");
		...
		if ($data->num_rows > 0) {
			...
			$connection->query("UPDATE register SET token='$str', expire = DATE_ADD(NOW(), INTERVAL 5 MINUTE) WHERE email='$email'");
